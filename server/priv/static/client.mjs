...
// build/dev/javascript/shared/shared/sql.mjs
var GetClippingsRow = class extends CustomType {
  constructor(id, user_id, text3) {
    super();
    this.id = id;
...
// build/dev/javascript/shared/shared/decoders.mjs
function get_clippings_row_decoder() {
  return field(
    "id",
    string2,
    (id) => {
      return field(
        ...
        (user_id) => {
          return field(
            ...
            (text3) => {
              return success(
                new GetClippingsRow(id, user_id, text3)
              );
            }
          );
        }
...
function clippings_decoder() {
  let _pipe = get_clippings_row_decoder();
  return list2(_pipe);
}
...
// build/dev/javascript/shared/shared/pages/main.mjs
var Clipping = class extends CustomType {
  constructor(id, text3, selected) {
    super();
    this.id = id;
    this.text = text3;
...
var UserSelected = class extends CustomType {
  constructor(id) {
    super();
    this.id = id;
  }
};
function view_clippings(rows) {
  return ul(
    toList([]),
    (() => {
      let _pipe = rows;
      return map(
        ...
        (row) => {
          ...
          return button(
            toList([
              on_click(new UserSelected(row.id)),
              class$(color)
            ]),
            toList([text2(row.text)])
          );
        }
      );
    })()
...
function init2(flags) {
  let clippings = (() => {
    ...
    return map(
      _pipe,
      (c) => {
        return new Clipping(c.id, c.text, false);
      }
    );
  })();
  return [new MainPage(clippings), none()];
}
function update(model, msg) {
  {
    let x = model.clippings;
    let v = msg.id;
    return [
      new MainPage(
        (() => {
          ...
          return map(
            ...
            (c) => {
              ...
              let _record = c;
              return new Clipping(_record.id, _record.text, selected);
            }
          );
        })()
      ),
...
function view(model) {
  {
    let c = model.clippings;
    return view_clippings(c);
  }
}
function main() {
  let json_ = (() => {
    ...
    return try$(
      _pipe$1,
      (x) => {
        let _pipe$2 = parse(x, clippings_decoder());
        return replace_error(_pipe$2, void 0);
      }
    );
  })();
  let flags = (() => {

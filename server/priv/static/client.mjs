...
function init2(flags) {
  debug(flags);
  return [new MainPage(flags), none()];
}
function update(model, msg) {
  if (model instanceof MainPage && msg instanceof Incr) {
    let v = model.value;
    return [new MainPage(v + 1), none()];
  } else {
    let v = model.value;
    return [new MainPage(v - 1), none()];
  }
...
function main() {
  let json_ = (() => {
    ...
    let _pipe$1 = map(_pipe, innerText);
    return try$(
      _pipe$1,
      (x) => {
        let _pipe$2 = parse(x, int2);
